...
function Agricolacard() {
  const [dados, setDados] = useState(null);
  const [carregando, setCarregando] = useState(false);
  const [erro, setErro] = useState(null);
  const [mostrar, setMostrar] = useState(false);

  const executarAgricola = () => {
    if (mostrar) {
      setMostrar(false);
      return;
    }
  ...
  return (
    <div className="cont">
      <h2 className="text-xl font-bold mb-4 text-center">Agrícola</h2>
      <img src={img} alt="" srcset="" />
      <button
        onClick={executarAgricola}
        className="bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 transition w-full"
        disabled={carregando}
      >
        {carregando ? "Carregando..." : mostrar ? "Esconder" : "Ver Agrícola"}
      </button>
      <br/>
