...
function RefeitorioCard() {
  const [dados, setDados] = useState(null);
  const [carregando, setCarregando] = useState(false);
  const [erro, setErro] = useState(null);
  const [mostrar, setMostrar] = useState(false);

  const executarRefeitorio = () => {
    if (mostrar) {
      setMostrar(false);
      return;
    }
    ...
    axios.get("http://localhost:5000/refeitorio")
      .then(res => {
        setDados(res.data);
        setMostrar(true);
      })
  ...
  return (
    <div className="cont">
      <h2 className="text-xl font-bold mb-4 text-center">Refeitório</h2>
      <img src={img} alt="refeitorio" />
      <button
        onClick={executarRefeitorio}
        className="bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 transition w-full"
        disabled={carregando}
      >
        {carregando ? "Carregando..." : mostrar ? "Esconder" : "Ver Refeitório"}
      </button>
      <br />
